...
public class SettingsPanel extends JPanel {
    ...
    private static final String DEFAULT_GROQ_MODEL = "qwen/qwen3-32b";
    ...
    public SettingsPanel(MontoyaApi api) {
        ...
        gbc.weightx = 1.0;
        providerCombo = new JComboBox<>(new String[]{PROVIDER_OLLAMA, PROVIDER_GROQ});
        ...
        gbc.gridwidth = 2;
        JLabel modelsHint = new JLabel("<html><small>Groq: llama-3.1-8b-instant, llama-3.3-70b-versatile, qwen/qwen3-32b</small></html>");
    ...
    private void loadSettings() {
        try {
            ...
            if (provider != null && provider.equals(PROVIDER_GROQ)) {
                providerCombo.setSelectedItem(PROVIDER_GROQ);
            } else {
            ...
            // Load Groq API Key
            String apiKey = prefs.getString(PREF_GROQ_API_KEY);
            if (apiKey != null && !apiKey.isEmpty()) {
                groqApiKeyField.setText(apiKey);
            }
    ...
    private void saveSettings() {
        try {
            // Save provider
            prefs.setString(PREF_PROVIDER, (String) providerCombo.getSelectedItem());
            
            // Save Ollama URL
            prefs.setString(PREF_OLLAMA_URL, ollamaUrlField.getText().trim());
            
            // Save Groq API Key
            prefs.setString(PREF_GROQ_API_KEY, new String(groqApiKeyField.getPassword()).trim());
    ...
    private void updateProviderUI() {
        ...
        if (isOllama) {
            ...
            if (currentModel != null && !isGroqModel(currentModel)) {
    ...
    private boolean isGroqModel(String model) {
        for (String groqModel : GROQ_MODELS) {
            if (groqModel.equals(model)) return true;
        }
        return false;
    }
    ...
    public AiProvider getAiProvider() {
        ...
        if (PROVIDER_GROQ.equals(providerCombo.getSelectedItem())) {
     ...
     */
    public boolean isGroqProvider() {
        return PROVIDER_GROQ.equals(providerCombo.getSelectedItem());
    }
